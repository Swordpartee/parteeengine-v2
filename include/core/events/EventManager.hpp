...
template <typename EventType> using TypedSubscriber = std::function<void(const EventType)>;
} // namespace detail
...
template <typename EventType> void EventManager::subscribe(const detail::TypedSubscriber<EventType> subscriber) {
...
template <typename EventType> void EventManager::emit(const EventType event) const {
