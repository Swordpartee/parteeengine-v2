...
namespace std
{
	...
	[[nodiscard]] it_type next(it_type it, const typename std::iterator_traits<it_type>::difference_type distance = 1)
	{
		it.advance(distance);
		return it;
	}
	...
	template <plf::hive_iterator_concept it_type>
	[[nodiscard]] it_type prev(it_type it, const typename std::iterator_traits<it_type>::difference_type distance = 1)
	{
		it.advance(-distance);
		return it;
	}
	...
	template <plf::hive_iterator_concept it_type>
	typename std::iterator_traits<it_type>::difference_type distance(const it_type first, const it_type last)
	{
	...
	template <plf::hive_iterator_concept it_type>
	typename std::iterator_traits<it_type>::difference_type distance(const std::move_iterator<it_type> first, const std::move_iterator<it_type> last)
	{
		return first.base().distance(last.base());
	}
...
private:
	...
	struct group
	{
		...
		bool no_erasures() const noexcept
		{
		...
		aligned_pointer_type first_element() const noexcept
		{
		...
		aligned_pointer_type start() const noexcept
		{
		...
		aligned_pointer_type past_back() const noexcept
		{
	...
	void check_capacities_conformance(const hive_limits capacities) const
	{
		...
		if (capacities.min < hard_capacities.min || capacities.min > capacities.max || capacities.max > hard_capacities.max)
		{
				...
				throw std::invalid_argument("Supplied memory block capacity limits are either invalid or outside of block_capacity_hard_limits()");
			#else
				std::terminate();
			#endif
		}
...
private:
	...
	constexpr void destroy_element([[maybe_unused]] const aligned_pointer_type element) noexcept
	{
	...
	void destroy_remainder([[maybe_unused]] const_iterator it) noexcept
	{
...
public:
	...
	// Range insert, move_iterator overload (allows hive to use the faster internal overload for std::distance, otherwise the standard libraries will use the bidirectional std::distance overload):
...
public:
	...
	iterator erase(const const_iterator &iterator1, const const_iterator &iterator2)	// if uninitialized/invalid iterators supplied, function could generate an exception. If iterator1 > iterator2, behaviour is undefined.
	{
		...
		if (iterator1.group_pointer != iterator2.group_pointer)
		{
			...
			// ========================================================
			if (current.element_pointer != current.group_pointer->first_element()) // most common case
			{
...
public:
	...
	// Range assign, move_iterator overload (logic same as range insert, move_iterator):
	...
	void trim_capacity(const size_type capacity_retain) noexcept
	{
		...
		for (group_pointer_type current_group = unused_groups_head, previous_group = nullptr; current_group != nullptr;)
		{
			...
			if (capacity_to_remove_remaining >= current_group->capacity)
			{
				...
				if (capacity_to_remove_remaining < min_block_capacity)
				{
					break;
				}
			}
...
private:
	...
	hive_iterator<is_const> get_it(const pointer element_pointer) const noexcept
	{
		if (end_iterator.group_pointer != nullptr)
		{
			...
			if (std::greater_equal()(aligned_element_pointer, end_iterator.group_pointer->start()) && std::less()(aligned_element_pointer, end_iterator.element_pointer))
			{
			...
			{
				if (std::greater_equal()(aligned_element_pointer, current_group->start()) && std::less()(aligned_element_pointer, current_group->past_back()))
				{
...
public:
	...
	private:
		...
		void set_to_start_of_group() noexcept
		{
		...
		void set_to_first_element_in_group() noexcept // TODO: try moving these functions
		{
		...
		void set_to_past_back_of_group() noexcept
		{
	...
	public:
		...
		template <plf::hive_iterator_concept it_type>
		friend typename std::iterator_traits<it_type>::difference_type std::distance(const std::move_iterator<it_type> first, const std::move_iterator<it_type> last);
		...
		hive_iterator & operator ++ ()
		{
			assert(group_pointer != nullptr); // covers uninitialised hive_iterator
				...
				if (element_pointer == group_pointer->past_back() && group_pointer->next_group != nullptr) // Second condition allows iterator to reach end(), which may be 1 past end of block, if block has been fully used
				{
					group_pointer = group_pointer->next_group;
					group_pointer->set_to_first_element_in_group();
				}
			#else
				...
				{
					group_pointer = group_pointer->next_group;
					const aligned_pointer_type elements = group_pointer->start(); // TODO : try optimization here
	...
	private:
		...
		void set_element_location_from_skipfield() noexcept
		{
		...
		void advance_from_group_beginning(skipfield_type distance) noexcept
		{
		...
		aligned_pointer_type get_end() const noexcept
		{
			#ifndef PLF_HIVE_DISABLE_EXPENSIVE_BOUNDING
				if (group_pointer->next_group == nullptr)
				{
					hive_iterator current(group_pointer, nullptr, 0);
			...
			if (distance > 0) // ie. +=
			{
				...
				if (element_pointer != group_pointer->first_element()) // Otherwise we skip this section and treat the first block as we would an intermediary block
				{
					...
					if (group_pointer->no_erasures())
					{
						...
						else if (group_pointer->next_group == nullptr) // either we're going to reach end() or go beyond it, so bound to back of block
						{
							element_pointer = end;
				...
				{
					if (group_pointer->next_group == nullptr) // either we would reach end() or go beyond it, so bound to end() - necessary to reach rbegin()
					{
		...
		difference_type distance(const hive_iterator &last) const
		{
			...
			if (iterator1.group_pointer != iterator2.group_pointer) // if not in same group, process intermediate groups
			{
				...
				else if (iterator1.element_pointer == iterator1.group_pointer->first_element()) // rare case
				{
					distance += static_cast<difference_type>(iterator1.group_pointer->size);
				}
				else // Manually iterate to find distance to end of group:
				{
					const skipfield_pointer_type endpoint = iterator1.skipfield_pointer + (iterator1.group_pointer->past_back() - iterator1.element_pointer);
			...
			if (iterator2.group_pointer->no_erasures()) // ie. we can do simple subtraction
			{
	...
	public:
		...
		typedef typename hive::value_type 		value_type;
		typedef typename hive::difference_type	difference_type;
		typedef typename std::conditional<is_const_r, typename hive::const_pointer, typename hive::pointer>::type		pointer;
		typedef typename std::conditional<is_const_r, typename hive::const_reference, typename hive::reference>::type	reference;
		...
		template <bool is_const_rit = is_const_r, class = typename std::enable_if<is_const_rit>::type >
		...
		{
			return hive_iterator<is_const_r>(current);
		}
